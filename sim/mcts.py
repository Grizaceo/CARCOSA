...
def _run_rollout(
    ...
    while not state.game_over and depth < max_depth:
        ...
        if action is None:
            ...
            action = Action(
                actor=state.turn_order[state.turn_pos]
...
def mcts_search(
    ...
    actor = (
        root_state.turn_order[root_state.turn_pos]
        if root_state.phase == "PLAYER"
        else "KING"
    )
    ...
    for i in range(num_rollouts):
        ...
        if not state.game_over and node.untried_actions:
            ...
            if not next_state.game_over:
                next_actor = (
                    next_state.turn_order[next_state.turn_pos]
                    if next_state.phase == "PLAYER"
                    else "KING"
                )

                if str(next_actor) == player_id:
                    # It's our turn again: Expand ALL options
                    child_node.untried_actions = get_legal_actions(
        ...
        rollout_reward = _run_rollout(
            state, cfg, rng.fork(f"rollout_{i}"), rollout_policy_fn, max_depth
        )
