...
def card_priority(card_id: str) -> int:
    ...
    if "KEY" in card_upper or "LLAVE" in card_upper:
        return PRIORITY_KEY
    if "MONSTER" in card_upper or any(
        m in card_upper
        for m in [
            "ARAÑA",
            "SPIDER",
...
class CardMemory:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.box_id == other.box_id
            and self.position_in_deck == other.position_in_deck
        )
...
class BotMemory:
    ...
    def add_memory(self, card: CardMemory) -> Optional[CardMemory]:
        ...
        # Evitar duplicados
        if any(
            c.card_id == card.card_id and c.box_id == card.box_id
            for c in self.remembered_cards
        ):
            return None
        ...
        worst_idx = max(
            range(len(self.remembered_cards)),
    ...
    def remove_memory(self, card_id: str) -> None:
        ...
        self.remembered_cards = [
            c for c in self.remembered_cards if c.card_id != card_id
        ]
...
class TeamMemory:
    ...
    room_for_box: Dict[str, str] = field(
        default_factory=dict
    )  # box_id → room_id (inverso)
    ...
    assignments: Dict[str, Set[str]] = field(
        default_factory=dict
    )  # player_id → {card_ids}
    ...
    def share_card(self, card: CardMemory, from_player: str) -> None:
        ...
        existing = next(
            (
                ...
                if c.card_id == card.card_id and c.box_id == card.box_id
            ),
            None,
        )
        if existing:
            ...
            existing.seen_step = (
                card.seen_step if card.seen_step >= 0 else existing.seen_step
            )
            existing.seen_round = (
                card.seen_round if card.seen_round >= 0 else existing.seen_round
            )
            existing.observation_type = (
                card.observation_type or existing.observation_type
            )
            ...
            if card.source_player and card.source_player != existing.source_player:
                ...
                existing.confidence = min(
                    1.5, max(existing.confidence, card.confidence)
                )
            return
    ...
    def mark_card_removed(self, card_id: str) -> None:
        ...
        self.known_cards = [
            c for c in self.known_cards if c.card_id not in self.removed_cards
        ]
    ...
    def get_best_targets(self, priority_filter: Optional[int] = None) -> List[str]:
        ...
        sorted_cards = sorted(
            cards, key=lambda c: (c.priority, c.rounds_since_seen, -c.confidence)
        )
