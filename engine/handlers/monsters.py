...
MonsterPostSpawnHandler = Callable[
    [GameState, PlayerId, MonsterState, Config, Optional[RNG]], None
]
...
def register_monster_spawn(
    monster_id: str,
) -> Callable[[MonsterSpawnHandler], MonsterSpawnHandler]:
...
def register_monster_reveal(
    monster_id: str,
) -> Callable[[MonsterRevealHandler], MonsterRevealHandler]:
...
def try_monster_spawn(
    state: GameState, pid: PlayerId, monster_id: str, cfg: Config, rng: Optional[RNG]
) -> bool:
...
def _spawn_tue_tue(
    ...
    if rev == 1:
        ...
        if amount > 0:
            apply_sanity_loss(
                state, p, amount, source="TUE_TUE_3", cfg=cfg, apply_vanidad=False
            )
    return True
...
def _reveal_spider(
    ...
    add_status(
        ...
        "TRAPPED",
        duration=3,
        metadata={"source_monster_id": monster_id},
    )


@register_monster_reveal("SPIDER")
def _reveal_spider_en(
    state: GameState, pid: PlayerId, monster_id: str, cfg: Config, rng: Optional[RNG]
) -> None:
    add_status(
        state.players[pid],
...
def _post_spawn_sack(
    ...
    p.statuses.append(
        StatusInstance(
            status_id="TRAPPED",
            remaining_rounds=3,
            metadata={"source_monster_id": monster.monster_id},
        )
    )
    ...
    candidates = [
        r for r in canonical_room_ids() if r not in occupied_rooms and r != start_room
    ]
