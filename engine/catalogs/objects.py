...
OBJECT_CATALOG = {
    ...
    "COMPASS": ObjectDefinition(
        "COMPASS", "Brújula", uses=1, is_blunt=False, can_react=True
    ),
    ...
    "BLUNT": ObjectDefinition(
        "BLUNT", "Objeto Contundente", uses=1, is_blunt=True, can_react=False
    ),
    ...
    "PORTABLE_STAIRS": ObjectDefinition(
        "PORTABLE_STAIRS", "Escalera Portátil", uses=1, is_blunt=False
    ),
    # --- CUENTOS DE AMARILLO (4 objetos, mecánicamente iguales) ---
    "TALE_REPAIRER": ObjectDefinition(
        "TALE_REPAIRER", "El Reparador de Reputaciones", uses=None, is_blunt=False
    ),
    "TALE_MASK": ObjectDefinition("TALE_MASK", "La Máscara", uses=None, is_blunt=False),
    "TALE_DRAGON": ObjectDefinition(
        "TALE_DRAGON", "En la Corte del Dragón", uses=None, is_blunt=False
    ),
    "TALE_SIGN": ObjectDefinition(
        "TALE_SIGN", "El Signo de Amarillo", uses=None, is_blunt=False
    ),
    # --- LIBRO (Soulbound) ---
    "BOOK_CHAMBERS": ObjectDefinition(
        "BOOK_CHAMBERS", "El Rey de Amarillo", uses=None, is_soulbound=True
    ),
    # --- TESOROS (de Motemey) ---
    "TREASURE_RING": ObjectDefinition(
        "TREASURE_RING", "Llavero", uses=None, is_blunt=False, is_treasure=True
    ),
    "TREASURE_STAIRS": ObjectDefinition(
        "TREASURE_STAIRS", "Escaleras Tesoro", uses=3, is_blunt=False, is_treasure=True
    ),
    "TREASURE_SCROLL": ObjectDefinition(
        "TREASURE_SCROLL", "Pergamino", uses=None, is_blunt=False, is_treasure=True
    ),
    "TREASURE_PENDANT": ObjectDefinition(
        "TREASURE_PENDANT", "Colgante", uses=None, is_blunt=False, is_treasure=True
    ),
    # --- TESOROS SOULBOUND ---
    # Corona: soulbound desde inicio, activa Falso Rey
    "CROWN": ObjectDefinition(
        "CROWN",
        "Corona",
        uses=None,
        is_blunt=False,
