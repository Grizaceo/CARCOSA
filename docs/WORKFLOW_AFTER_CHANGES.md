...
python tools/run_versioned.py --all-seeds
python tools/analyze_version.py       # p=0.290, UNIFORM ✓
```
...
python tools/run_versioned.py --all-seeds
# Creates: runs/runs_v<NEW_HASH>_main_<time>/
python tools/analyze_version.py       # Compare with previous
```
...
─────────────────────     ─────────────────────
runs/runs_v4fee5ba.../         runs_archive/
...
**System Version:** runs/runs_v4fee5ba (RNG fix + versioning)
