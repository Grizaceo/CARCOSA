...
class CarcosaDataset(Dataset):
    ...
    def __init__(
        ...
        self.observations = torch.tensor(
            self.df[self.OBS_COLS].values, dtype=torch.float32
        )
        self.actions = torch.tensor(self.df["action_id"].values, dtype=torch.long)

        print(
            f"Dataset cargado: {len(self)} ejemplos, {self.obs_dim} features, {self.num_actions} acciones"
        )
...
def create_dataloaders(
    ...
    dataset = CarcosaDataset(
        csv_path, filter_policy=filter_policy, filter_outcome=filter_outcome
    )
