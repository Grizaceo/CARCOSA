...
def main():
    ...
    logging.info(
        f"cfg: gens={args.generations} mutants={args.mutants} "
    ...
    for gen in range(1, args.generations + 1):
        ...
        with ctx.Pool(args.workers) as pool:
            ...
            f_muts = [
                pool.apply_async(worker_mutate_train_eval, (t[1],))
                for t in tasks
                if t[0] == "mut"
            ]
            ...
            logging.info(
                f"  [ANCLA] madre: {Path(mother).name} -> {mother_wr * 100:.1f}%"
            )
            ...
            for i, f in enumerate(f_muts, 1):
                ...
                logging.info(
                    f"  mutante {i}/{len(f_muts)} -> {res['wr'] * 100:.1f}% "
        ...
        logging.info(
            f"  >> madre ancla: {mother_wr * 100:.1f}% -> "
        ...
        if best_wr >= args.objective_wr:
            logging.info(
                f"  >> OBJETIVO {args.objective_wr * 100:.0f}% ALCANZADO en gen {gen}"
            )
            break
        ...
        if gen >= args.fallback_gen:
            ...
            if not any_win:
                logging.info(
                    f"  >> FALLBACK: tras gen {gen} sigue 0% victorias. "
    ...
    for h in history:
        logging.info(
            f"  Gen {h['gen']}: mejor {h['best_win_rate'] * 100:.1f}% "
