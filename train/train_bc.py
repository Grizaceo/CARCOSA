...
def train_bc(data_path="train/goldset_500_wins.pkl", model_save_path="models/bc_goldset_v1", epochs=200, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs} - Loss: {avg_train_loss:.4f} - Train Acc: {train_acc:.2%} - Val Acc: {val_acc:.2%} - LR: {current_lr}")
        ...
        if val_acc > best_val_acc:
            ...
            print(f"--> New best Val Acc: {val_acc:.2%}. Model saved to {model_save_path}_best.zip")
        else:
            patience_counter += 1
