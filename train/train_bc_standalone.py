...
def main():
    ...
    print(
        f"Dataset: {obs.shape[0]} muestras, obs_dim={obs.shape[1]}, num_actions={len(CarcosaEnv.ACTION_TYPES)}"
    )

    hidden = [int(x) for x in args.hidden.split(",")]
    model = CarcosaPolicyNet(
        obs_dim=obs.shape[1],
        num_actions=len(CarcosaEnv.ACTION_TYPES),
        hidden_sizes=hidden,
    ).to(args.device)
    ...
    for ep in range(args.epochs):
        ...
        if (ep + 1) % 5 == 0 or ep == 0:
            print(
                f"  ep {ep + 1}/{args.epochs}  loss={tot / n:.4f}  acc={correct / n:.3f}"
            )
