...
def main():
    ...
    print("=" * 72)
    print(f"CONTRA-FÁCTICO SETUP — GOAL idéntica, solo cambia el setup ({n} seeds, {dt:.0f}s)")
    print("=" * 72)
    rows = []
    for v in VARIANTS:
        rs = by_var[v]
        ...
        print(f"  {v:<14} n={len(rs):>3} wins={len(wins):>3}  {wr:>5.1f}%  "
    ...
    for v in VARIANTS[1:]:
        ...
        print(f"  {v}: novelas vs base={len(novel)} {novel[:12]} | perdidas vs base={len(lost)} {lost[:8]}")

    out = {
        "meta": {"seeds": n, "workers": args.workers, "tag": args.tag},
        "results": results,
        "summary": {v: {"wins": len([r for r in by_var[v] if r["win"]]), "n": len(by_var[v])} for v in VARIANTS},
    }
    out_path = repo / "reports" / f"setup_counterfactual_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
