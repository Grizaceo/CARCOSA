...
class CarcosaPolicyNet(nn.Module):
    ...
    def sample_action(
        self, obs: torch.Tensor, temperature: float = 1.0
    ) -> torch.Tensor:
        """
...
class CarcosaTransformerPolicy(nn.Module):
    ...
    def __init__(
        ...
        self.output = nn.Sequential(
            nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, num_actions)
        )
...
class CarcosaActorCritic(nn.Module):
    ...
    def __init__(
        ...
        for size in policy_sizes:
            policy_layers.extend([nn.Linear(prev_size, size), nn.ReLU()])
            prev_size = size
        ...
        for size in value_sizes:
            value_layers.extend([nn.Linear(prev_size, size), nn.ReLU()])
            prev_size = size
