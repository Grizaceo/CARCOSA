...
class CarcosaEnv(gym.Env):
    ...
    def __init__(
        ...
        self.curriculum_keys34_max_keys = max(
            self.curriculum_keys34_min_keys, int(curriculum_keys34_max_keys)
        )
        self.curriculum_keys34_fragile_sanity_min = int(
            curriculum_keys34_fragile_sanity_min
        )
        ...
        self.curriculum_keys34_fragile_carriers = max(
            0, int(curriculum_keys34_fragile_carriers)
        )
        self.curriculum_closing_prob = max(
            0.0, min(1.0, float(curriculum_closing_prob))
        )
        self.curriculum_keys_start = max(0, int(curriculum_keys_start))
        self.curriculum_far_player_prob = max(
            0.0, min(1.0, float(curriculum_far_player_prob))
        )
        ...
        self.spawn_key_distribution_jitter = max(
            0.0, min(1.0, float(spawn_key_distribution_jitter))
        )
    ...
    def _team_umbral_distance(self, state) -> int:
        return sum(
            self._distance_to_umbral(state, player.room)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        if self.state and not self.state.game_over and self.state.phase != "KING":
            ...
            if actor_str in self.state.players:
                ...
                king_floor_match = (
                    1.0 if floor_of(p.room) == self.state.king_floor else 0.0
                )
                ...
                has_stairs = (
                    1.0 if any("TREASURE_STAIRS" in item for item in p.objects) else 0.0
                )
                ...
                if hints:
                    ...
                    def dist_to(target_room):
                        if str(p.room) == str(target_room):
                            return 0
                        queue = deque([(p.room, 0)])
                        visited = {p.room}
                        while queue:
                            curr, d = queue.popleft()
                            for nb in self._room_neighbors_with_stairs(
                                self.state, curr
                            ):
                                if nb in visited:
                                    continue
                                if str(nb) == str(target_room):
                                    return d + 1
                                visited.add(nb)
                                queue.append((nb, d + 1))
                        return 999
                ...
                key_here = (
                    1.0 if any(h["room_id"] == str(p.room) for h in hints) else 0.0
                )
                ...
                critical_team = sum(
                    1 for pl in self.state.players.values() if pl.sanity < -2
                ...
                keys_missing = (
                    4.0 - sum(pl.keys for pl in self.state.players.values())
                ) / 4.0
        ...
        room_by_id = (
            {str(rid): room for rid, room in self.state.rooms.items()}
            if self.state
            else {}
        )
        for nid in ALL_NODES:
            if room_by_id and nid in room_by_id:
                ...
                special_revealed = (
                    1.0 if getattr(rm, "special_revealed", False) else 0.0
                )
                special_destroyed = (
                    1.0 if getattr(rm, "special_destroyed", False) else 0.0
                )
            else:
        ...
        for pid in ["P1", "P2", "P3", "P4"]:
            if pid in players:
                ...
                status_count = min(
                    1.0, float(len(getattr(p, "statuses", []) or [])) / 3.0
                )
                has_stairs = (
                    1.0
                    if any(
                        "TREASURE_STAIRS" in str(o)
                        for o in (getattr(p, "objects", []) or [])
                    )
                    else 0.0
                )
                king_risk = (
                    ...
                    if (
                        self.state
                        and floor_of(p.room) == self.state.king_floor
                        and p.sanity <= 0
                    )
                    else 0.0
                )
        ...
        obs = np.array(
            base_obs + local_obs + node_block + player_block + global_block,
            dtype=np.float32,
        )
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        umbral_distances = [
            self._distance_to_umbral(self.state, player.room)
        ...
        return {
            ...
            "requested_action_type": self.last_action_debug.get(
                "requested_action_type"
            ),
            "requested_action_legal": self.last_action_debug.get(
                "requested_action_legal"
            ),
            "requested_action_out_of_range": self.last_action_debug.get(
                "requested_action_out_of_range", False
            ),
            ...
            "requested_action_matched_executed": self.last_action_debug.get(
                "requested_action_matched_executed"
            ),
            "action_selection_source": self.last_action_debug.get(
                "action_selection_source"
            ),
            "fallback_substitution": self.last_action_debug.get(
                "fallback_substitution", False
            ),
            "fallback_substitution_reason": self.last_action_debug.get(
                "fallback_substitution_reason"
            ),
            "peek_available": self.last_action_debug.get("peek_available", False),
            "illegal_action_intent": self.last_action_debug.get(
                "illegal_action_intent", False
            ),
        }
    ...
    def _select_deterministic_fallback_action(
        ...
        ranked = sorted(
            legal_actions,
            key=lambda action: (
                self.ACTION_TYPES.index(action.type)
                if action.type in self.ACTION_TYPES
                else 999
            ),
        )
        return ranked[0]
    ...
    def _apply_closing_curriculum(self) -> None:
        ...
        farthest_room = max(
            all_rooms, key=lambda rid: self._distance_to_umbral(self.state, rid)
        )
        close_rooms = sorted(
            all_rooms, key=lambda rid: self._distance_to_umbral(self.state, rid)
        )
    ...
    def _apply_keys34_curriculum(self) -> None:
        ...
        keys_target = self.rng.randint(
            self.curriculum_keys34_min_keys, self.curriculum_keys34_max_keys
        )
        player_order = list(players)
        self.rng.shuffle(player_order)
        ...
        if candidate_rooms:
            ...
            other_pool = (
                far_sorted_rooms[split_idx:]
                if far_sorted_rooms[split_idx:]
                else far_sorted_rooms
            )
    ...
    def _room_for_box(self, state) -> Dict[str, str]:
        return {
            str(box_id): str(room_id) for room_id, box_id in state.box_at_room.items()
        }

    def _pick_action_for_type(
        self, legal_actions: List[Action], target_type: Optional[ActionType], actor: str
    ) -> Optional[Action]:
    ...
    def _pick_hint_followup_action(
        self, state, legal_actions: List[Action], actor: str
    ) -> Optional[Action]:
        ...
        actor_room = str(
            state.players[next(pid for pid in state.players if str(pid) == actor)].room
        )
        for hint in hints:
            if hint["room_id"] == actor_room:
                search_candidates = [
                    a for a in legal_actions if a.type == ActionType.SEARCH
                ]
            ...
            move_candidates = [
                ...
                and str(a.data.get("to", "")) == hint["room_id"]
            ]
            if move_candidates:
    ...
    def _pick_guided_fallback_action(
        self, state, legal_actions: List[Action], actor: str
    ) -> Optional[Action]:
        if not legal_actions:
            return None
    ...
    def _extract_observations(
        self, state, action: Action, actor: str
    ) -> List[Dict[str, Any]]:
        if actor not in state.players:
            return []
    ...
    def _register_observations(
        self, observations: List[Dict[str, Any]], actor: str, state_round: int
    ) -> float:
        reward = 0.0
            ...
            if entry is None:
                ...
                if actor != entry.get("source_player"):
                    entry["confidence"] = min(
                        1.5, float(entry.get("confidence", 1.0)) + 0.1
                    )
    ...
    def _compute_info_usage_reward(
        ...
        if not hints:
            if action.type == ActionType.SKIP_PEEK and any(
                a.type == ActionType.PEEK_ROOM_DECK for a in legal_actions
            ):
        ...
        actor_room = str(
            prev_state.players[
                next(pid for pid in prev_state.players if str(pid) == actor)
            ].room
        )
        move_destinations = {
            str(a.data.get("to", ""))
        ...
        if action.type == ActionType.MOVE:
            ...
            used_hint = next(
                (hint for hint in hints if hint["room_id"] == actor_room), None
            )
        ...
        if used_hint is not None:
            ...
            if curr_keys > prev_keys:
                reward += (
                    self.reward_info_realize
                    * used_hint["effective_value"]
                    * source_factor
                )
                if entry is not None:
                    entry["resolved"] = True
    ...
    def reset(
        ...
        closing_cutoff = min(
            1.0, self.curriculum_keys34_prob + self.curriculum_closing_prob
        )
    ...
    def _apply_spawn_randomization(self) -> None:
        ...
        if (
            self.spawn_key_distribution_jitter > 0.0
            and local_rng.random() < self.spawn_key_distribution_jitter
        ):
    ...
    def step(
        ...
        legal: List[Action] = []
        requested_action_out_of_range = action_id < 0 or action_id >= len(
            self.ACTION_TYPES
        )
        requested_action_type = (
            self.ACTION_TYPES[action_id]
            if 0 <= action_id < len(self.ACTION_TYPES)
            else None
        )
        requested_action_label = (
            requested_action_type.value
            if requested_action_type is not None
            else "OUT_OF_RANGE"
        )
        illegal_action_intent = False
        masked_out_action = False
        ...
        if pending:
            ...
            requested_action_legal = (
                requested_action_type in legal_types
                if requested_action_type is not None
                else False
            )
            masked_out_action = (
                requested_action_type is not None
                and requested_action_type not in legal_types
            )
            ...
            if requested_action_type is not None and requested_action_type not in {
                a.type for a in legal
            }:
                illegal_action_intent = True
            if action is None:
                action = self._select_deterministic_fallback_action(
                    legal, actor, ActionType.ACCEPT_SACRIFICE
                )
                action_selection_source = (
                    "PENDING_DETERMINISTIC_FALLBACK"
                    if legal
                    else "PENDING_DEFAULT_ACCEPT"
                )
                fallback_substitution_reason = (
                    "masked_out" if masked_out_action else "no_direct_type"
                )
            ...
            if action is None:
                action = self._select_deterministic_fallback_action(
                    legal, actor, ActionType.END_TURN
                )
                action_selection_source = (
                    "DETERMINISTIC_FALLBACK" if legal else "DEFAULT_END_TURN"
                )
                fallback_substitution_reason = (
                    "masked_out" if masked_out_action else "no_direct_type"
                )
        ...
        fallback_substitution = actor != "KING" and (
            requested_action_type is None or requested_action_type != action.type
        )
        ...
        self.last_action_debug = {
            ...
            "requested_action_matched_executed": requested_action_type == action.type
            if requested_action_type is not None
            else False,
    ...
    def _calculate_reward(
        ...
        if next_state.game_over:
            reward = (
                self.reward_win if next_state.outcome == "WIN" else self.reward_lose
            )
        else:
            reward = 0.0
            ...
            if (
                self.penalty_existence_per_round > 0
                and next_state.round > prev_state.round
            ):
            ...
            if curr_team_umbral_dist < prev_team_umbral_dist:
                reward += self.reward_phase2_umbral_progress * (
                    prev_team_umbral_dist - curr_team_umbral_dist
                )
            elif curr_team_umbral_dist > prev_team_umbral_dist:
                reward += self.penalty_phase2_umbral_regress * (
                    curr_team_umbral_dist - prev_team_umbral_dist
                )

            prev_near_umbral = sum(
                1
                for player in prev_state.players.values()
                if self._distance_to_umbral(prev_state, player.room) <= 1
            )
            curr_near_umbral = sum(
                1
                for player in next_state.players.values()
                if self._distance_to_umbral(next_state, player.room) <= 1
            )
            if curr_near_umbral > prev_near_umbral:
                reward += self.reward_phase2_sync_step * (
                    curr_near_umbral - prev_near_umbral
                )
    ...
    def render(self):
        ...
        if self.render_mode == "human" or self.render_mode == "ansi":
            ...
            for pid, p in self.state.players.items():
                output.append(
                    f"  {pid}: Room={p.room}, Sanity={p.sanity}, Keys={p.keys}"
                )
