...
def main():
    ...
    for ep in range(args.episodes):
        ...
        touch_umbral.append(any(str(p.room) == str(env.cfg.UMBRAL_NODE) for p in st.players.values()))
        rounds_death.append(st.round)
        oc = info.get("outcome", "TIMEOUT")
        outcomes[oc] = outcomes.get(oc, 0) + 1

    print(f"Episodios: {args.episodes}")
    print(f"Llaves promedio al final: {np.mean(keys_at_end):.2f}  (max visto: {max(keys_at_end)})")
