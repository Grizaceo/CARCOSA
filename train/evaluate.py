...
def run_evaluation_episode(
    ...
    while step_idx < max_steps and not state.game_over:
        ...
        if action is None:
            actor = (
                str(state.turn_order[state.turn_pos])
                if state.phase == "PLAYER"
                else "KING"
            )
            if actor == "KING":
...
def compare_policies(episodes: int = 50):
    ...
    for r in sorted(all_results, key=lambda x: -x["win_rate"]):
        print(
            f"{r['policy']:<15} {100 * r['win_rate']:>9.1f}% {r['avg_steps']:>12.1f} {r['avg_rounds']:>12.1f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default=None, help="Ruta al modelo .pt para evaluar"
    )
    parser.add_argument("--episodes", type=int, default=50, help="Número de episodios")
    parser.add_argument(
        "--compare", action="store_true", help="Comparar todas las policies heurísticas"
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    if args.compare:
        ...
        results = evaluate_policy(
            policy, f"NN:{Path(args.model).stem}", args.episodes, cfg
        )
        print_results(results)
    else:
        print("Uso:")
